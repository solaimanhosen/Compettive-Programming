...
class Solution {
    ...
    void print_st(stack<int> st, vector<int> nums, ofstream& file) {
        if (st.empty()) return;
        int tp = st.top();
        st.pop();
        print_st(st, nums, file);
        file << "[" << tp << ", " << nums[tp] << "]" << endl;
    }
public:
    int maximumScore(vector<int>& nums, int k) {
        ...
        stack<int> temp_st = stack<int>(st);
        ofstream outFile1;
        outFile1.open("output.txt");
        print_st(temp_st, nums, outFile1);
        outFile1.close();

        ofstream outFile2 = ofstream("output1.txt");
        ...
        for (int i = k + 1; i < nums.size(); ++i) {
            while (!st.empty() && nums[i] < nums[st.top()]) {
                auto top = st.top();
                st.pop();
                outFile2 << "top: " << nums[top] << " " << st.top() << " ";
                if (st.top() <= k - 1) max_score = max(max_score, nums[top] * (st.empty() ? i : i - (st.top() + 1)));
                // st.pop();
                outFile2 << "[" << i << ", " << max_score << "]" << endl;
            }
            st.push(i);
        }
        ...
        while (!st.empty()) {
            ...
            // max_score = max(max_score, top.first * (nums.size() - top.second));
            max_score = max(max_score, nums[top] * (st.empty() ? nums.size() : (nums.size() - (st.top() + 1))));
            // st.pop();
        }
...
int main() {
    ...
    for (int i = 0; i < nums.size(); ++i) {
        outfile << "[" << i << ", " << nums[i] << "]    " << 36246.00/nums[i] << endl;
    }
    outfile.close();
    cout << (new Solution())->maximumScore(nums, k) << endl;
}
